<template>
  <div class="container">
    ...
    <button v-on:click="StorageAdd()">storage</button>
    <div v-show="false">
    <svg ref="svgCard" width="1200" height="630" style="background-color:#ddd">
    ...
    AddStore: function () {
      ...
      .then(async docRef => {
        ...
        for (let index = 0; index < this.book_card_data.length; index++) {
          await db.collection("cards").doc(docRef.id).collection('datas').add(this.book_card_data[index])
        }
        this.StorageAdd(docID)
      })
